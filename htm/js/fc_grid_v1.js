...
var FCGrid$ = function () {
    ...
    magicZoomFC: function(id, novoArray, novoArrayAmp, FC_MaxImages, refreshZoom){
      ...
      for (var i=0;i<=FC_MaxImages;i++)
      {
        if(i===0)
        {
          ...
          sHtmlZoom+="<a href="+imgAmpMini+" title=\""+ fn.getNameProduct() +"\" class=MagicZoomPlus id=zoom2 rel=\"selectors-class:active; zoom-width:400px; zoom-height:400px; selectors-change:mouseover;\"><img src="+ imgDetMini +"></a><br>"
                    +"<p><a class=\"FCGridBtnZoom\" onclick=\"MagicZoomPlus.expand(zoom2); return false;\" href=\"#\"><img src='"+ FC$.PathImg +"botampliar.svg' border='0' /></a></p>"
                    +"<a href=\""+imgAmpMini+"\"  rel=\"zoom-id:zoom2;\" rev=\""+ imgDetMini +"\"><img src=\""+ imgDetMini +"\" width=60 height=60 class=ZoomIMG2></a>";
        }
        else{
          imgDetMini=FC$.PathPrd+novoArray[i];
          imgAmpMini=FC$.PathPrd+novoArrayAmp[i];
          sHtmlZoom+="<a href="+imgAmpMini+" rel='zoom-id:zoom2;' rev="+ imgDetMini +"><img src="+ imgDetMini +" width=60 height=60 class=ZoomIMG2></a>";
        }
      }
    ...
    setPriceProduct: function(product){
      ...
      if(parseFloat(product.priceNum) > 0 && oPositionPrice){
        ...
        if(product.priceNum!=product.priceOri){
           return oPositionPrice.innerHTML="<div style='font-size:18px; color:#a6a6a6'>de <strike>"+FCLib$.FormatPreco(FormatPrecoReais(product.priceOri))+"</strike></div><div style='font-size:20px; color:#000000'>por <font style='font-size:24px;font-weight:700;'>"+FCLib$.FormatPreco(FormatPrecoReais(product.priceNum))+"</font></div>" + oMaxInstallments + oEconomyJS;
         }
         else{
           return oPositionPrice.innerHTML="<div style='font-size:20px; color:#000000'>por <font style='font-size:24px;font-weight:700;'>"+FCLib$.FormatPreco(FormatPrecoReais(product.priceNum))+"</font></div>"+ oMaxInstallments;
         }
  ...
  function fnExecTabDescriptors(){
    ...
    for(var i=0; i< aDados.length; i++ ){ var THnode = document.createElement("th"); THnode.style.border="1px solid #dadada"; THnode.style.padding="3px 8px"; THnode.innerHTML = [aDados[i]]; TRnode.appendChild(THnode)}
    oTable.appendChild(TRnode);

    for(var i=0; i < aProductList.length;i++){
      var TRnode = document.createElement("tr");
      var prd = JSON.parse(aProductList[i]);
      for(var j=0; j< aDados.length; j++ ){ var TDnode = document.createElement("td"); TDnode.style.border="1px solid #dadada"; TDnode.style.padding="3px 8px"; TDnode.innerHTML = prd[aDados[j]]; TRnode.appendChild(TDnode);}
      oTable.appendChild(TRnode);
    }
